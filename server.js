...
async function sendOTP(email, otp) {
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `<div style="font-family:sans-serif;max-width:400px;margin:auto;padding:30px;border:1px solid #eee;border-radius:12px"><h2>Mã xác thực OTP</h2><p>Đây là mã OTP của bạn:</p><div style="font-size:36px;font-weight:bold;letter-spacing:8px;color:#e94560;text-align:center;padding:20px;background:#f5f5f5;border-radius:8px;margin:20px 0">${otp}</div><p style="color:#888;font-size:13px">Mã có hiệu lực trong <strong>5 phút</strong>.</p></div>`,
    }),
  });
}
...
app.post('/api/register/send-otp', async (req, res) => {
  ...
  if (!email || !password || !name) return res.json({ success: false, message: 'Vui lòng điền đầy đủ thông tin.' });
  const users = getUsers();
  if (users.find(u => u.email === email)) return res.json({ success: false, message: 'Email này đã được đăng ký.' });
...
app.post('/api/register/verify-otp', async (req, res) => {
  ...
  if (Date.now() > record.expires) { delete otpStore[email]; return res.json({ success: false, message: 'OTP đã hết hạn.' }); }
...
app.get('/api/me', (req, res) => { if (req.session.user) res.json({ loggedIn: true, user: req.session.user }); else res.json({ loggedIn: false }); });
